# The SEC with local regression for Optdigits dataset
...
best_params, best_acc = param_tuning_sec(
    ...
    param_grid_gammal=param_grid_gammal,  # fix gamma_l if we're not using local
    k=k,
    normalized=True,
    use_local_regression=True  # using local regression for now
)
print("Best params (mu, gamma_g):", best_params)
